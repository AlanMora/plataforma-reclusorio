...
| P12 | **Centro de origen en traslados** | El Modelo de Datos v1.0 no define un centro de origen en `traslados`; el origen se INFERÍA del último INGRESO de la persona, solo para pintar el mapa (P11) | **APROBADA (18/09/2026, indicación directa del equipo):** columna `idCentroOrigen` (uuid, NOT NULL) en `traslados`, capturada con un select del catálogo `centros` en el alta y validada en el backend como catálogo activo. Migración `1786700000000-TrasladoCentroOrigen`: rellena los registros existentes con el centro del último ingreso/egreso no descartado de cada persona y ABORTA (sin aplicar nada, misma transacción) si algún traslado pertenece a una persona sin ingreso registrado. `GET /traslados/mapa` pasa a leer la columna en vez de inferir el origen. El alta precarga el select con el centro donde está la persona (`idCentroActual`, derivado, expuesto en `GET /personas/:id`) |
