        ...
        <table class="tabla">
          ...
          <tbody>
            @for (a of archivos(); track a.idArchivo) {
              <tr>
                <td class="max-w-[220px] truncate" [title]="a.nombreOriginal">{{ a.nombreOriginal }}</td>
                <td><span class="chip-neon">{{ a.tipoArchivo }}</span></td>
                <td class="font-mono text-xs">{{ formatoBytes(a.tamanoBytes) }}</td>
                <td class="max-w-[140px] truncate font-mono text-[10px] text-slate-500" [title]="a.hashSha256">
                  {{ a.hashSha256 }}
                </td>
                <td class="font-mono text-xs">{{ a.fechaRegistro | date: 'dd/MM/yy HH:mm' }}</td>
                <td class="text-right">
                  <button class="btn-secundario btn-mini" type="button" (click)="descargar(a)">Descargar</button>
                  <button
